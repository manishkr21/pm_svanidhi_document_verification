...
# ----------------------------------------------------
# ⚙️ Sidebar Navigation & Module Selection
# ----------------------------------------------------
st.sidebar.markdown("<h2 style='color: #1E1E1E;'>🛡️ Control Center</h2>", unsafe_allow_html=True)
...
# ----------------------------------------------------
# 1️⃣ AI / Deepfake Image Detection View
# ----------------------------------------------------
if selected_module == "🔍 AI / Deepfake Image Detection":
    ...
    st.sidebar.markdown("### ⚙️ AI Model Settings")
    ...
    st.sidebar.info("ℹ️ **Model Info**: Uses PyTorch MobileNetV3 classifier trained for GAN & Deepfake face detection.")
    ...
    if image_to_process is not None:
        ...
        with col2:
            ...
            if not model:
                st.error("⚠️ Model file (`mobilenet_best (3).pth`) not found or could not be loaded!")
            else:
                if face_detected:
                    st.success("✅ Face detected! Analyzing facial features...")
                    target_image = cropped_face
                else:
                    st.warning("⚠️ No distinct face detected. Analyzing overall image...")
...
# ----------------------------------------------------
# 2️⃣ QR Code Detection & Decoding View
# ----------------------------------------------------
    ...
    st.sidebar.info("ℹ️ Uses OpenCV QR Code Detector to find matrix locations and decode payload content.")
    ...
    if image_to_process is not None:
        ...
        with col2:
            ...
            if qr_detected:
                ...
                """, unsafe_allow_html=True)
           # ----------------------------------------------------
# 3️⃣ Edited / Manipulated Image Detection View
# ----------------------------------------------------
elif selected_module == "✏️ Edited Image Detection":
    st.markdown("<h1 style='text-align: center; color: #E53935;'>✏️ Multi-Signal Image Forensics Detector</h1>", unsafe_allow_html=True)
    ...
    st.sidebar.info("ℹ️ Powered by TruFor Deep Learning (CMX + Noiseprint++) fused with Adaptive ELA, Noise Outliers, DCT Energy, Edge Density, ORB Copy-Move, and EXIF/XMP Metadata.")
    ...
    if image_to_process is not None:
        ...
        st.markdown("### 🗺️ Visual Localization & Forensic Maps")
        ...
        st.markdown(f"""
            <div class='result-box'>
                <h2 style='color: {"#E53935" if is_edited else "#4CAF50"};'>{"⚠️ Edit / Manipulation Detected" if is_edited else "✅ Image Appears Unedited"}</h2>
        ...
        if details.get('soft_list'):
            st.warning(f"⚠️ Editing Signature / Software Tag Detected: `{', '.join(details['soft_list'])}`")
